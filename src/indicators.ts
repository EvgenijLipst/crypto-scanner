...
import { OHLCVRow, TechnicalIndicators, MIN_HISTORY_CANDLES, MIN_VOLUME_SPIKE, MAX_RSI_OVERSOLD } from './types';
 ...
 */
export function calculateIndicators(candles: OHLCVRow[]): TechnicalIndicators | null {
  if (candles.length < MIN_HISTORY_CANDLES) {
    log(`Not enough candles: ${candles.length} < ${MIN_HISTORY_CANDLES}`, 'WARN');
    return null;
  }
  ...
  try {
    ...
    log(`Error calculating indicators: ${error}`, 'ERROR');
    return null;
  }
}
 ...
 */
export function checkBuySignal(indicators: TechnicalIndicators): boolean {
  const { bullishCross, volSpike, rsi } = indicators;
  
  // Все условия должны выполняться одновременно
  return bullishCross && 
         volSpike >= MIN_VOLUME_SPIKE && 
         rsi < MAX_RSI_OVERSOLD;
}
...
export function formatIndicators(indicators: TechnicalIndicators): string {
  return [
    ...
    `Volume Spike: x${indicators.volSpike.toFixed(1)} ${indicators.volSpike >= MIN_VOLUME_SPIKE ? '✅' : '❌'}`,
    `RSI: ${indicators.rsi.toFixed(1)} ${indicators.rsi < MAX_RSI_OVERSOLD ? '✅' : '❌'}`
  ].join('\n');
} 